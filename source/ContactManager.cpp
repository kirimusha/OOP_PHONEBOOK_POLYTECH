...
bool ContactManager::addContact(const Contact& contact) const {
    return m_repository->addContact(contact);
}

bool ContactManager::removeContact(const string& email) const {
    return m_repository->removeContact(email);
}

bool ContactManager::updateContact(const string& oldEmail, const Contact& contact) const {
    return m_repository->updateContact(oldEmail, contact);
}
...
bool ContactManager::updateAllContacts(const vector<Contact>& contacts) const {
